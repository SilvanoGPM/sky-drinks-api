...
class ApplicationUserControllerIT {
    ...
    void verifyByEmail_VerifyIfUserExists_WhenSuccessful() {
        ApplicationUser savedUser = applicationUserRepository.save(ApplicationUserCreator.createApplicationUserToBeSave());

        ResponseEntity<Void> entity = testRestTemplate.getForEntity("/users/verify-by-email/{email}", Void.class, savedUser.getEmail());
